...
class GitCommitLinker:
  ...
  def __init__(self, repoId):
    ...
    """
    self.repo_path = os.path.join(os.path.dirname(__file__), '..', self.REPO_DIR + repoId['name'])
    self.repo_id = repoId
  ...
  def linkCorrectiveCommits(self, corrective_commits, all_commits):
    ...
    for i in range(corrective_commits.shape[0]):
      corrective_commit = corrective_commits.iloc[i]
    ...
    for i in range(all_commits.shape[0]):
      commit = all_commits.iloc[i]
      if commit['commit_hash'] in linked_commits:
        all_commits.loc[i,'contains_bug'] = True
        all_commits.loc[i,'fixes'] = json.dumps(linked_commits[commit['commit_hash']])
    return all_commits
