        ...
        for i in range(df_commits.shape[0]):
            ...
            for index,row in  df_committed_file[df_committed_file['commit_id'] == df_commits.iloc[i,0]].iterrows():
                ...
                if len(row['file_path'].split('src/')) == 1:
                    ...
                    continue
                temp = row['file_path']
                b = temp.split('src/')[1].replace('/', '.').rsplit('.', 1)[0]
...
#obj = MetricsGetter("https://github.com/ThrowTheSwitch/Unity", "Unity", "C")
#obj = MetricsGetter("https://github.com/libjpeg-turbo/libjpeg-turbo", "libjpeg-turbo", "C")
obj = MetricsGetter("https://github.com/openresty/lua-nginx-module", "lua-nginx-module", "C")
